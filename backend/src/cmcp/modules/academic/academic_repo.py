...
class AcademicRepo:
    """
    Domain repository for academic module.
    Uses BaseRepository for generic CRUD + pagination,
    plus custom existence/link checks here.
    """
    ...
    def faculty_name_exists(self, *, company_id: int, name: str, exclude_id: Optional[int] = None) -> bool:
        conds = [
            ...
            func.lower(Faculty.name) == func.lower(name.strip()),
        ]
        if exclude_id:
    ...
    def faculty_code_exists(self, *, company_id: int, code: str, exclude_id: Optional[int] = None) -> bool:
        ...
        conds = [
            Faculty.company_id == int(company_id),
            func.lower(Faculty.code) == func.lower(code.strip()),
        ]
        if exclude_id:
            conds.append(Faculty.id != int(exclude_id))
        stmt = select(exists().where(*conds))
    ...
    def department_name_exists(
        self, *, company_id: int, faculty_id: int, name: str, exclude_id: Optional[int] = None
    ) -> bool:
        conds = [
    ...
    def department_code_exists(self, *, company_id: int, code: str, exclude_id: Optional[int] = None) -> bool:
        ...
        conds = [
            Department.company_id == int(company_id),
            func.lower(Department.code) == func.lower(code.strip()),
        ]
        if exclude_id:
            conds.append(Department.id != int(exclude_id))
        stmt = select(exists().where(*conds))
    ...
    def semester_name_exists(self, *, company_id: int, name: str, exclude_id: Optional[int] = None) -> bool:
        ...
        conds = [
            ...
            func.lower(Semester.name) == func.lower(name.strip()),
        ]
        if exclude_id:
    ...
    def semester_number_exists(
        self, *, company_id: int, academic_year_id: int, number: int, exclude_id: Optional[int] = None
    ) -> bool:
        conds = [
            Semester.company_id == int(company_id),
    ...
    def course_code_exists(self, *, company_id: int, code: str, exclude_id: Optional[int] = None) -> bool:
        ...
        conds = [
            Course.company_id == int(company_id),
            func.lower(Course.code) == func.lower(code.strip()),
        ]
        if exclude_id:
            conds.append(Course.id != int(exclude_id))
        stmt = select(exists().where(*conds))
    ...
    def chapter_title_exists(
        self, *, company_id: int, course_id: int, title: str, exclude_id: Optional[int] = None
    ) -> bool:
        conds = [
    ...
    def chapter_number_exists(
        self, *, company_id: int, course_id: int, number: int, exclude_id: Optional[int] = None
    ) -> bool:
        conds = [
            Chapter.company_id == int(company_id),
            Chapter.course_id == int(course_id),
    ...
    # ----------------------------
    # Linked record checks (prevent delete)
    # ----------------------------
