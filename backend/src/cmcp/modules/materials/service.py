...
class MaterialsService:
    ...
    # =========================================================
    # CREATE (UPDATED for CourseOffering)
    # =========================================================
    def create_material(
            self,
            *,
        ...
        # CHANGED: Use course_offering_id instead of course_id
        ...
        # CHANGED: Check title exists in offering scope
        if self.repo.title_exists_in_scope(
                company_id=company_id,
                course_offering_id=course_offering_id,  # CHANGED
                chapter_id=chapter_id,
                title=title,
        ):
            return False, ERR_MATERIAL_TITLE_EXISTS, None
    ...
    # =========================================================
    # UPDATE (UPDATED for CourseOffering)
    # =========================================================
    def update_material(
            self,
            *,
        ...
        # ❌ cannot change these
        # CHANGED: Check course_offering_id instead of course_id
        ...
        if "title" in data and data["title"] is not None:
            ...
            if new_title.strip().lower() != (material.title or "").strip().lower():
                ...
                if self.repo.title_exists_in_scope(
                        company_id=company_id,
                        course_offering_id=int(material.course_offering_id),  # CHANGED
                        chapter_id=int(material.chapter_id) if material.chapter_id else None,
                        title=new_title,
                        exclude_id=int(material.id),
                ):
        ...
        try:
            with self.s.begin_nested():
                ok, msg, _ = self.material_svc.update(company_id=company_id, id=material_id, data=patch,
                                                      return_public=False)
                if not ok:
                    raise BusinessValidationError(msg)
