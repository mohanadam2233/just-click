...
const MaterialDetailsPrimary = ({ id }) => {
  ...
  const readHref = normalizeUrl(
    material?.file?.readUrl || rawMaterial?.file?.read_url || "",
  );
  ...
  const canReadFile = Boolean(readHref && readHref !== "#");
  const canDownloadFile =
    Boolean(downloadHref && downloadHref !== "#") && isDownloadable;
  ...
  const handleOpenRead = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        alert("Could not open material. Please try again.");
        return;
      }
      ...
      console.error("Open material failed:", error);
      alert("Could not open material. Please check your connection.");
    }
  };
          ...
          <div className="lg:col-span-4">
            <div className="sticky top-28 space-y-6">
              ...
              >
                ...
                <div className="space-y-3 text-sm">
                  {[
                    ["Type", material?.file?.extension || "—"],
                    ["Size", formatFileSize(material?.file?.sizeMb)],
