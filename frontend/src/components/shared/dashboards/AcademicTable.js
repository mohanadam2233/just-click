...
//           if (!cellVal.includes(String(searchVal).toLowerCase())) {
//             return false;
//           }
//         }
//       }
...
//                 className="p-1.5 text-gray-500 dark:text-gray-400 bg-white dark:bg-slate-800 border border-gray-200 dark:border-slate-700 rounded hover:bg-gray-50 dark:hover:bg-slate-700 transition-colors disabled:opacity-50"
//               >
//                 <RefreshIcon />
//               </button>
...
//                     onChange={(val) =>
//                       handleFilterChange(col.filterKey || col.key, val)
//                     }
...
//                     }
//                   />
//                   {filters[col.filterKey || col.key] && (
//                     <button
//                       onClick={() =>
//                         handleFilterChange(col.filterKey || col.key, "")
//                       }
...
const AcademicTable = ({
  ...
  const handleFilterChange = (key, rawValue) => {
    const value = getFilterValue(rawValue);
  ...
  const filterColumns = useMemo(() => {
    const configured = columns.filter(
      (col) => col.filterKey || col.filterDropdown || col.filterType,
    );

    return configured.length > 0 ? configured : columns.slice(0, 4);
  }, [columns]);
        ...
        <div className="flex items-center gap-2">
          {selectedIds.length > 0 ? (
            ...
            <>
              ...
              <button
                type="button"
                className="p-1.5 text-gray-500 dark:text-gray-400 bg-white dark:bg-slate-800 border border-gray-200 dark:border-slate-700 rounded hover:bg-gray-50 dark:hover:bg-slate-700 transition-colors"
              >
                <MenuDotsIcon />
              </button>
            </>
        ...
        <div className="flex gap-2 flex-wrap max-w-4xl items-center">
          {filterColumns.map((col) => {
            ...
            if (col.filterDropdown) {
              ...
              return (
                <div key={filterKey} className="w-40 relative group">
                  <AsyncDropdown
          ...
          <thead>
            <tr className="border-b border-gray-200 dark:border-slate-800">
              ...
              {columns.map((col) => (
                <th
                  key={col.key}
                  className={`py-3 px-2 text-xs font-medium text-gray-500 relative select-none ${
              ...
              paginatedData.map((row, rowIdx) => {
                ...
                return (
                  <tr
                    key={rowId}
                    onClick={() => onRowClick && onRowClick(row)}
                    className={`border-b border-gray-100 dark:border-slate-800/70 group transition-colors hover:bg-gray-50 dark:hover:bg-slate-800/50 ${
